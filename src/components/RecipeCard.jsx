...
export default function RecipeCard({ recipe, onOpen, onToggleSave, isSaved }) {
  ...
  const openSource = () => {
    const url = recipe.strSource || recipe.source || recipe.strYoutube || recipe.url;
    if (url) window.open(url, "_blank", "noopener,noreferrer");
  };

  return (
    <div className="card recipe">
      <div className="thumb" onClick={() => onOpen?.(recipe)} role="button">
        {img ? (
      ...
      <div className="actions">
        ...
        <button className="btn btn-soft" onClick={openSource}>Open</button>
      </div>
    </div>
  );
}
